...
pbc = rng.uniform(low=0.2, high=10.0, size=(1, 3))
test_coords_1 = rng.uniform(low=0.2, high=10.0, size=(rows, 3))
test_coords_2 = rng.uniform(low=0.2, high=10.0, size=(rows, 3))

print(f"Start Execution with {rows} atoms")
...
print(f"Arrays match: {np.allclose(frame_results_C, frame_results_Cu)}")
